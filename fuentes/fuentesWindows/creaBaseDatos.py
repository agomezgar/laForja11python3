...
class baseDatos:
    ...
    def rellena_matriculas(self, archivo):
        ...
        print("intentando...")
        try:
            f=open(archivo,'r',newline='',encoding='utf-8')
            #next (f,None)
            print(f)
...
#Rellenamos tabla prioridades:
    ...
    def buscaMateriasCurso(self,curso):
        try:
            ...
            for row in valor.fetchall():
                tablas = self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;")
                #print(str(row[1]))
                for t in tablas.fetchall():
                    #print (t[0])
                    if((row[1]+'organizacionEstandares') in t[0]):
                        result[row[1]]=row[0]
            #print(result)
    ...
    def buscaMateriasCursonuevaProgramacion(self, curso):
        try:
            consulta = "SELECT materia,codigo FROM materias WHERE curso=(?) ORDER BY materia"
            # print("Buscando materias para el curso: ")
            # print(curso)
