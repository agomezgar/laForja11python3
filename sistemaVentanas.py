...
class ventana:
    ...
    def justificarCalificaciones(self):
        ...
        def dameNotas(event):
            ...
            if not(trimestre==4):
                consulta="SELECT * FROM "+nombreTabla+" WHERE trimestre=="+str(trimestre)+" ORDER BY idestandar"
            else:
                consulta="SELECT * FROM "+nombreTabla+" ORDER BY idestandar"
                
            #print ("Consulta: "+consulta)
            ejconsulta=base.cursor.execute(consulta)
            estandares=ejconsulta.fetchall()
...
#GENERAMOS UN PDF
            ...
            cabecera2=estilos['Heading2']
            cabecera3=estilos['Heading3']
    ...
    def generaProgramacion(self):
        ...
        def eligeCurso(event):
            ...
            if (self.combo.current()!=0):
                ...
                base= baseDatos('laForja.db')
                self.curso=self.combo.current()
                self.materia=base.buscaMateriasCurso(self.curso)
                self.combo2["values"]=sorted(list(self.materia.values()))
                base.close()
